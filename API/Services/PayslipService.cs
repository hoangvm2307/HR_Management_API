...
namespace API.Services
{
    public class PayslipService
    {
        ...
        public async Task<PayslipDTO> AddPayslip(
                int staffId,
            ...
            )
        {
            ...
            int PretaxEarning = CalculatePretaxEarning(Insurance, grossSalary);
            ...
            int taxableIncome = TaxableIncomeCalculation(PretaxEarning, TotalAllowancesDeduction);

            //Thue Thu Nhap Ca Nhan
            var result = PersonalIncomeTaxCalculate(taxableIncome);

            int PersonalIncomeTax = (int)result.Total();

            //Net Salary
            int NetSalary = PretaxEarning - PersonalIncomeTax;
        ...
        public static int TaxableIncomeCalculation(int PretaxEarning, int TotalAllowancesDeduction)
        {
        ...
        public static int CalculatePretaxEarning(InsuranceDTO Insurance, int grossSalary)
        {
            return ((int)
                (grossSalary - Insurance.SocialInsurance - Insurance.HealthInsurance - Insurance.UnemploymentInsurance));
        }
        ...
        {
            return await _context.Payslips.AnyAsync(c => c.StaffId == staffId && c.PayslipId == payslipId);
        }

        public async  Task<PayslipDTO> GetPayslipOfStaffByPayslipId(int staffId, int payslipId)
        {
            var payslip = await _context.Payslips.Where(c => c.StaffId == staffId && c.PayslipId == payslipId).FirstOrDefaultAsync();
